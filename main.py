...
def has_unsupported_chars(text, font_size=40):
    try:
        font = ImageFont.truetype("PingFangTC-Medium.otf", font_size)
    except:
        return list(text)
...
def create_image(text, output_path="output.png"):
    ...
    while font_size > 20:
        main_font = ImageFont.truetype("PingFangTC-Medium.otf", font_size)
        too_wide = False
        for line in lines:
    ...
    main_font = ImageFont.truetype("PingFangTC-Medium.otf", font_size)
    watermark_font = ImageFont.truetype("PingFangTC-Medium.otf", watermark_font_size)
